...
## Load GFW Fishing Effort at 0.1 degrees for 2020-2024

fisheries <- rast("data/Effort_Fishing_01deg_2020-2024.tif")
names(fisheries) <- paste0(c("Effort2020", "Effort2021", "Effort2022", "Effort2023", "Effort2024"))

## Load GFW Fishing Effort by Gear for 2024

# gear2024file <- 'data/Effort_FishingByGear_01deg_2024.rds'
gear2024file <- 'data/Effort_FishingByGear_01deg_2024.tif'

if(grepl("\\.rds$", gear2024file)){
  gear2024 <- stack(readRDS(gear2024file))
  names(gear2024) <- sub(".*_[0-9]{4}_(.*)", "\\1", names(gear2024))
  names(gear2024) <- paste0(toupper(substr(names(gear2024), 1, 1)), substr(names(gear2024), 2, nchar(names(gear2024))))
  }else{
    gear2024 <- stack(gear2024file)
    gear2024names = read.csv('data/Effort_FishingByGear_01deg_2024_names.csv')
    names(gear2024) = gear2024names$names
}
  ...
  titlePanel(h1("South Pacific TurtleWatch Model"), windowTitle = "SPTW Telemetry Model"),
    navbarPage(title = div(img(src = "upwell_green_gray.png", style="margin-top:0px;padding-left:4px;padding-bottom:10px;padding-top:2px", height = 55), "Eastern Pacific Leatherback Movement", style = "margin-top:-13px"),
               ...
               tabPanel("Interactive Map",
                   ...
                   tags$head(
                     tags$style(HTML(
                       ".checkbox {margin:0;text-align:left}
                        ...
                        .leaflet-control-layers label {margin-top: 0px; margin-bottom: 0px}
                        "))),
                   selectInput(inputId = "FisheriesYear", label = "Yearly Fishing Effort (hr/km^2)", choices = c("2024" = 5, "2023" = 4, "2022" = 3, "2021" = 2, "2020" = 1), selected = "2024"),
                   ...
                   hr(style="margin-top:15px;margin-bottom:15px;border-color: #cd6ebe;opacity:0.4;border-top: #cd6ebe dashed 1.5px;"),
                   selectInput(inputId = "Fishinggear", label = "2024 Fishing Effort by Gear (hr/km^2)", choices = c("Drifting longlines" = 2, "Tuna purse seines" = 15, "Squid jigger" = 12, "Purse seines" = 8, "Other purse seines" = 5, "Fishing" = 3, "Pole and line" = 6, "Set longlines" = 11, "Trawlers" = 13, "Trollers" = 14, "Set gillnets" = 10, "Dredge fishing" = 1, "Fixed gear" = 4, "Pots and traps" = 7, "Seiners" = 9), selected = "Tuna_purse_seines"),
                   checkboxInput("PlotGear", "Plot 2024 Fishing Effort by Gear", FALSE),
            ...
            tabsetPanel(
                ...
                tabPanel('Chla',
                         ...
                         fluidRow(column(width = 1), column(
                           ...
                           column(p("Chlorophyll-a (mg/m\U00B3) along the Eastern Pacific with Exclusive Economic Zones (shown in gray)", style = "text-align:left;color:#A2B03A;padding:2px;font-size: 105%"), width = 12),
                           width = 12),
                           br(),
                         ))
...
server <- shinyServer(function(input,output,session) {
  ...
  filteredgear <- reactive({
     ...
     gear2024[[as.numeric(input$Fishinggear)]]
    ...
    if(input$PlotGear){
      ...
      observe({
        ...
        proxy %>%
          ...
          addLegend(pal = pal, values = terra::values(filteredgear()), title = paste("2024 Fishing Effort <br> ", sub("_"," ", names(filteredgear()))), group = "Fisheries.group", layer = "Fisherieslegend")})
